...
describe("createSession options merging", () => {
  ...
  it("omits CLAUDE_CODE_SUBAGENT_MODEL when no session model can be resolved", async () => {
    ...
    // The fork exports this var into the CLI env it spawns (and into dev
    // shells dogfooding it); without stubbing, it would leak in via the
    // `...process.env` spread and look like an injection.
